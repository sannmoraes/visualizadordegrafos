...
def atualizar_grafo(n_clicks_carregar_grafo, salvar_grafo, adicionar_vertice, remover_vertice, adicionar_aresta, remover_aresta, selectedNodeData,  tipo_grafo, peso_aresta, selectedEdgeData,  n_clicks_bfs, n_clicks_dfs):
    ...
    if ctx.triggered:
        ...
        if prop_id == 'carregar-grafo':
            ...
            if selectedEdgeData:
                print(f"Tipo de selectedEdgeData: {type(selectedEdgeData)}")
                print(f"Conteúdo de selectedEdgeData: {selectedEdgeData}")
                if isinstance(selectedEdgeData, list) and len(selectedEdgeData) > 0:
                    edge_data = selectedEdgeData[0]
                    if isinstance(edge_data, dict):                                      
                        ...
                        if source and target:
                            ...
                            print("Erro: 'source' ou 'target' não encontrados em edge_data.")
                    else:
                        print("Erro: edge_data não é um dicionário.")
                else:
                    print("Erro: selectedEdgeData não está no formato esperado ou está vazio.")              
